...
public class LocalidadRepositoryIT {
    ...
    public void givenExistLocalidad_thenUpdated() {
        ...
        final long LC_ID = 1L;
        final String LC_NM = "TEST";
        
        LocalidadRepository localidadRepository = new LocalidadRepository(dataSource);
        localidadRepository.setProvinciaRepository(provinciaRepository);
        
        Optional<LocalidadEntity> optional = localidadRepository.findById(LC_ID);
        assertThat(optional).isPresent();
        
        LocalidadEntity entity = optional.get();
        
        entity.setNombre(LC_NM);
        localidadRepository.save(entity);
        
        assertThat(entity.getNombre()).isEqualTo(LC_NM);
        
        Optional<LocalidadEntity> saved = localidadRepository.findById(LC_ID);
        assertThat(saved).isPresent().contains(entity);
        
        assertThat(saved.get().getNombre()).isEqualTo(LC_NM);
        
    }
    
    @Test
    @Transactional
    public void gicenExistsLocalidad_thenDelete() {
        final long LC_ID = 1L;
        LocalidadRepository localidadRepository = new LocalidadRepository(dataSource);
        Optional<LocalidadEntity> optional = localidadRepository.findById(LC_ID);
        assertThat(optional).isPresent();
        ...
        localidadRepository.delete(entity.getId());
        
        Optional<LocalidadEntity> deleted = localidadRepository.findById(LC_ID);
