...
public class ProvinciaRepositoryIT {
    ...
    public void givenExistProvincia_thenUpdated() {
        ...
        final long PR_ID = 1L;
        final String PR_NM = "TEST";
        
        ProvinciaRepository provinciaRepository = new ProvinciaRepository(dataSource);
        
        Optional<ProvinciaEntity> optional = provinciaRepository.findById(PR_ID);
        assertThat(optional).isPresent();
        
        ProvinciaEntity entity = optional.get();
        
        entity.setNombre(PR_NM);
        provinciaRepository.save(entity);
        
        assertThat(entity.getNombre()).isEqualTo(PR_NM);
        
        Optional<ProvinciaEntity> saved = provinciaRepository.findById(PR_ID);
        assertThat(saved).isPresent().contains(entity);
        
        assertThat(saved.get().getNombre()).isEqualTo(PR_NM);
        
    }
    
    @Test
    @Transactional
    public void gicenExistsProvincia_thenDelete() {
        final long PR_ID = 1L;
        ProvinciaRepository provinciaRepository = new ProvinciaRepository(dataSource);
        Optional<ProvinciaEntity> optional = provinciaRepository.findById(PR_ID);
        assertThat(optional).isPresent();
        ...
        provinciaRepository.delete(entity.getId());
        
        Optional<ProvinciaEntity> deleted = provinciaRepository.findById(PR_ID);
